...
ftx_message_rc_t ftx_message_encode(ftx_message_t* msg, ftx_callsign_hash_interface_t* hash_if, const char* message_text)
{
    ...
    // Fall back to free text (i3=0): anything that is not a valid standard or
    // nonstandard-call message is packed as up to 13 chars of free text. Uses
    // the ORIGINAL message_text (not the split call_to/de/extra tokens).
...
// Encode a free-text message (up to 13 chars from the 42-symbol table),
// message type FTX_MESSAGE_TYPE_FREE_TEXT (i3=0, n3=0). This is the inverse of
// ftx_message_decode_free: pack the (right-trimmed, upper-cased) text as a
// 71-bit base-42 integer, then left-shift by one bit so it occupies payload
// bits 0..70 with i3/n3 (bits 71..76) left zero. Added in this in-tree libft8
// copy (upstream ft8_lib does not provide a free-text encoder).
ftx_message_rc_t ftx_message_encode_free(ftx_message_t* msg, const char* text)
{
    // Take the last 13 characters (WSJT-X keeps the tail if longer) and map
    // each through the FT8_CHAR_TABLE_FULL alphabet; unknown chars become
    // space (index 0).
    char c14[14];
    int len = (int)strlen(text);
    // Right-justify into a 13-char field padded on the left with spaces, like
    // the decoder produces (leading zeros in base-42 == spaces).
    int start = (len > 13) ? (len - 13) : 0;
    int pad = (len < 13) ? (13 - len) : 0;
    for (int i = 0; i < 13; ++i)
    {
        if (i < pad)
            c14[i] = ' ';
        else
            c14[i] = to_upper(text[start + i - pad]);
    }
    c14[13] = '\0';

    // Build the 71-bit value in b71[9] (big-endian byte order): b71 = b71*42 + n
    uint8_t b71[9] = {0};
    for (int idx = 0; idx < 13; ++idx)
    {
        int n = nchar(c14[idx], FT8_CHAR_TABLE_FULL);
        if (n < 0)
            n = 0; // unknown char -> space
        // Multiply the 72-bit big-endian accumulator by 42 and add n.
        uint32_t carry = (uint32_t)n;
        for (int i = 8; i >= 0; --i)
        {
            uint32_t v = (uint32_t)b71[i] * 42u + carry;
            b71[i] = (uint8_t)(v & 0xFFu);
            carry = v >> 8;
        }
    }

    // Left-shift b71 by one bit into payload[0..9]; i3=0/n3=0 fill the low bits
    // of payload[9] (mirror of decode_telemetry's right-shift by one).
    uint8_t carry = 0;
    for (int i = 8; i >= 0; --i)
    {
        msg->payload[i] = (uint8_t)((b71[i] << 1) | carry);
        carry = (b71[i] >> 7) & 0x01u;
    }
    // payload[9]: top bit is the final carry (bit 70), i3/n3 (bits 71..76) = 0.
    msg->payload[9] = (uint8_t)(carry << 7);
